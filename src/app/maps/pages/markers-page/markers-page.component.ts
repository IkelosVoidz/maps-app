...
export class MarkersPageComponent implements AfterViewInit, OnDestroy {
  ...
  addMarker(lngLat: Coordinate, color: string): void {
    ...
    const samePositionFeature = this.features?.find(feature => {
        const coordinates = (feature.getGeometry() as Point).getCoordinates();
        const isEqual = coordinates[0] === lngLat[0] && coordinates[1] === lngLat[1];
        
        return isEqual;
    });

    console.log(samePositionFeature);
    
    if (samePositionFeature) {
        offsetLngLat = [
            lngLat[0] + (Math.random() - 0.5) / 10000,
            lngLat[1] + (Math.random() - 0.5) / 10000,
        ] as Coordinate;
    }
